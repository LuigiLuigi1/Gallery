...
setTimeout(() => {
    ...
    if (loader) {
        // fade out loader for a smoother transition then remove from flow
    ...
    // Reveal the grid container with a fade
    const grid = document.querySelector('.grid');
    if (grid) {
        grid.style.opacity = '1';
    }
    // Reveal grid items by selecting all .item elements
...
function checkScroll() {
    ...
    newFontSize = Math.max(1.5, Math.min(3, newFontSize));
